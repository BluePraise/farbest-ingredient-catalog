...
if ( $ingredient_id ) :
    ...
    // ACF fallback: same pattern as applications above.
    if ( empty( $fiber_list ) ) {
        $acf_fiber_value = get_field( 'product_fiber_benefits' );
        if ( is_array( $acf_fiber_value ) && ! empty( $acf_fiber_value ) ) {
            foreach ( $acf_fiber_value as $term_id ) {
                $term = get_term( intval( $term_id ), 'fpc_fiber_benefit' );
                if ( $term && ! is_wp_error( $term ) ) {
                    $fiber_list[] = $term->name;
                }
            }
        }
    }
    ...
    // ── Benefits columns: auto-merge taxonomy terms ──────────────────────────
    // If the ACF repeater has no column labelled "application" or "fiber",
    // inject synthetic columns so those taxonomy terms always appear.
    ...
    if ( ! empty( $benefits_columns ) ) {
        foreach ( $benefits_columns as $bc ) {
            ...
            if ( $lbl && false !== stripos( $lbl, 'application' ) ) {
                $has_app_column = true;
            }
            if ( $lbl && false !== stripos( $lbl, 'fiber' ) ) {
                $has_fiber_column = true;
            }
        }
    }
    ...
    $all_benefits_columns = array_merge(
        $injected_columns,
        is_array( $benefits_columns ) ? $benefits_columns : array()
    );
                ...
                <?php if ( ! empty( $all_benefits_columns ) ) : ?>
                    <div class="ingredient-benefits">
                        <?php foreach ( $all_benefits_columns as $column ) :
                            ...
                            if ( empty( $col_label ) && empty( $col_items ) ) {
                                continue;
                            }

                            // Collect manually-entered item texts for deduplication.
                            $manual_texts = array();
                            ...
                            // Map column label to the right taxonomy term list.
                            $auto_terms = array();
                            ...
                            // Prepend taxonomy terms not already present as manual items.
                            $merged_terms = array();
